...
class Encoder(nn.Module):
    def __init__(self, config, vis):
        ...
        self.vis = vis
        #nn.ModuleList 是 PyTorch 中的一个容器类，用于保存多个 nn.Module 对象。它类似于 Python 中的列表，但是被设计用于保存模块对象
...
class VisionTransformer(nn.Module):
    ...
    def load_from(self, weights):
        with torch.no_grad():
            ...
            # Encoder whole为每个block和unit加载权重
